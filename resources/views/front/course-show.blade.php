...
@section('content')
    <section class="max-w-4xl mx-auto px-6 pt-16 pb-24">
        <a href="{{ route('courses.public') }}" class="text-sm text-slate-500 hover:text-gold mb-6 inline-block">← กลับรายการคอร์ส</a>

        <div class="app-card p-6 sm:p-8 mb-6">
            @if ($course->subject)
                <span class="badge-brand mb-3">{{ $course->subject }} · {{ $course->grade_level }}</span>
            @endif
            <h1 class="text-3xl sm:text-4xl font-semibold text-white mb-4">{{ $course->title }}</h1>
            ...
            <div class="flex flex-wrap gap-4 text-sm text-slate-400 mb-6">
                ...
                <span class="font-bold text-gold text-base">
        ...
        @if ($course->lessons->isNotEmpty())
            <div class="app-card p-6">
                <h2 class="text-lg font-semibold text-white mb-4">บทเรียนในคอร์ส</h2>
                <ul class="space-y-2">
                    @foreach ($course->lessons as $lesson)
                        <li class="flex items-center gap-3 p-3 bg-white/5 text-sm">
                            <span class="w-7 h-7 flex items-center justify-center text-xs font-bold text-gold border border-gold/40">{{ $loop->iteration }}</span>
